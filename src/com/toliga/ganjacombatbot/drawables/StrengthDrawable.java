...
public class StrengthDrawable implements Drawable {

    private AbstractScript context;
    private Image image;

    public StrengthDrawable(AbstractScript context) {
        this.context = context;
        image = Utilities.LoadImage("http://www.account4rs.com/images/skill_powerleveling/strength.png", 15, 15);
    }
    ...
    public void draw(Graphics2D graphics) {
        ...
        graphics.drawString(context.getSkillTracker().getGainedExperiencePerHour(Skill.STRENGTH) + " XP", 420, 382); // XP / hr
        graphics.drawString(context.getSkillTracker().getGainedExperience(Skill.STRENGTH) + " XP", 420, 399); // XP gained

        long timeToLevelUp = context.getSkillTracker().getTimeToLevel(Skill.STRENGTH);
        ...
        int realLevel = context.getSkills().getRealLevel(Skill.STRENGTH);
        int boostedLevel = context.getSkills().getBoostedLevels(Skill.STRENGTH);
