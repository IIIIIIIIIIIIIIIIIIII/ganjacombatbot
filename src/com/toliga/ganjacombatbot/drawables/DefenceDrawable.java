...
public class DefenceDrawable implements Drawable {

    private AbstractScript context;
    private Image image;

    public DefenceDrawable(AbstractScript context) {
        this.context = context;
        image = Utilities.LoadImage("http://www.account4rs.com/images/skill_powerleveling/defence.png", 15, 15);
    }
    ...
    public void draw(Graphics2D graphics) {
        ...
        graphics.drawString(context.getSkillTracker().getGainedExperiencePerHour(Skill.DEFENCE) + " XP", 420, 382); // XP / hr
        graphics.drawString(context.getSkillTracker().getGainedExperience(Skill.DEFENCE) + " XP", 420, 399); // XP gained

        long timeToLevelUp = context.getSkillTracker().getTimeToLevel(Skill.DEFENCE);
        ...
        int realLevel = context.getSkills().getRealLevel(Skill.DEFENCE);
        int boostedLevel = context.getSkills().getBoostedLevels(Skill.DEFENCE);
