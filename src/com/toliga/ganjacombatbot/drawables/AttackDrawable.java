...
public class AttackDrawable implements Drawable {

    private AbstractScript context;
    private Image image;

    public AttackDrawable(AbstractScript context) {
        this.context = context;
        image = Utilities.LoadImage("http://www.account4rs.com/images/skill_powerleveling/attack.png", 15, 15);
    }
    ...
    public void draw(Graphics2D graphics) {
        ...
        graphics.drawString(context.getSkillTracker().getGainedExperiencePerHour(Skill.ATTACK) + " XP", 420, 382); // XP / hr
        graphics.drawString(context.getSkillTracker().getGainedExperience(Skill.ATTACK) + " XP", 420, 399); // XP gained

        long timeToLevelUp = context.getSkillTracker().getTimeToLevel(Skill.ATTACK);
        ...
        int realLevel = context.getSkills().getRealLevel(Skill.ATTACK);
        int boostedLevel = context.getSkills().getBoostedLevels(Skill.ATTACK);
